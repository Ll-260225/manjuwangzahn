...
export default function AdminPanel({ content, onContentChange }) {
  ...
  const [config, setConfig] = useState(
    () => loadCosConfig() || { secretId: "", secretKey: "" },
  );
  ...
  async function addProject(event) {
    ...
    try {
      ...
      if (video instanceof File && video.size) {
        setStatus("正在上传项目视频…");
        videoUrl = await uploadContentFile(video, "content/projects");
      }
      ...
      const next = {
        ...
        projects: [
          ...
          {
            id: crypto.randomUUID(),
            title: form.get("projectTitle"),
            type: form.get("projectType"),
            description: form.get("projectDescription"),
            coverUrl,
            videoUrl,
          },
        ],
  ...
  return (
    <aside className="admin-drawer" aria-label="内容管理">
      <header>
        <strong>内容管理</strong>
        <button
          type="button"
      ...
      <div className="admin-scroll">
        <p className="admin-note">
          桶：liwanmin-0115-1454067572（广州）。每次上传或新增项目都会自动写入 COS 内容清单。
        </p >
        ...
        <section>
          ...
          <input
            placeholder="SecretId（建议使用受限子账号）"
        ...
        <section>
          ...
          <label>
            ...
            <input
              type="file"
              accept="video/*"
              data-field="heroVideo"
              onChange={uploadSiteMedia}
            />
          </label>
